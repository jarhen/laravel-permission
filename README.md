...
<table>
   <tr>
      <td><img src="http://jarhen.github.io/laravel-permission/sponsor-logo.png"></td>
...
[![Latest Version on Packagist](https://img.shields.io/packagist/v/jarhen/laravel-permission.svg?style=flat-square)](https://packagist.org/packages/jarhen/laravel-permission)
[![Build Status](https://img.shields.io/travis/jarhen/laravel-permission/master.svg?style=flat-square)](https://travis-ci.org/jarhen/laravel-permission)
[![StyleCI](https://styleci.io/repos/42480275/shield)](https://styleci.io/repos/42480275)
[![Total Downloads](https://img.shields.io/packagist/dt/jarhen/laravel-permission.svg?style=flat-square)](https://packagist.org/packages/jarhen/laravel-permission)
...
Jarhen is a web design agency in Antwerp, Belgium. You'll find an overview of all
our open source projects [on our website](https://jarhen.be/opensource).
...
This package can be used in Laravel 5.4 or higher. If you are using an older version of Laravel, take a look at [the v1 branch of this package](https://github.com/jarhen/laravel-permission/tree/v1).
...
You can publish [the migration](https://github.com/jarhen/laravel-permission/blob/master/database/migrations/create_permission_tables.php.stub) with:
...
When published, [the `config/permission.php` config file](https://github.com/jarhen/laravel-permission/blob/master/config/permission.php) contains:
...
First, add the `Jarhen\Permission\Traits\HasRoles` trait to your `User` model(s):
...
The `role` scope can accept a string, a `\Jarhen\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
The scope can accept a string, a `\Jarhen\Permission\Models\Permission` object or an `\Illuminate\Support\Collection` object.
...
The `assignRole`, `hasRole`, `hasAnyRole`, `hasAllRoles`  and `removeRole` functions can accept a
 string, a `\Jarhen\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
string or a `Jarhen\Permission\Models\Permission` object.
...
All these responses are collections of `Jarhen\Permission\Models\Permission` objects.
...
1. It is best to flush the `jarhen.permission.cache` before seeding, to avoid cache conflict errors. This can be done from an Artisan command (see Troubleshooting: Cache section, later) or directly in a seeder class (see example below).
...
- Your `Role` model needs to extend the `Jarhen\Permission\Models\Role` model
- Your `Permission` model needs to extend the `Jarhen\Permission\Models\Permission` model

If you need to REPLACE the existing `Role` or `Permission` models you need to keep the
following things in mind:

- Your `Role` model needs to implement the `Jarhen\Permission\Contracts\Role` contract
- Your `Permission` model needs to implement the `Jarhen\Permission\Contracts\Permission` contract
...
The package doesn't come with any screens out of the box, you should build that yourself. To get started check out [this extensive tutorial](https://scotch.io/tutorials/user-authorization-in-laravel-54-with-jarhen-laravel-permission) by [Caleb Oki](http://www.caleboki.com/).
...
If you discover any security-related issues, please email [freek@jarhen.be](mailto:freek@jarhen.be) instead of using the issue tracker.

## Postcardware

You're free to use this package, but if it makes it to your production environment we highly appreciate you sending us a postcard from your hometown, mentioning which of our package(s) you are using.

Our address is: Jarhen, Samberstraat 69D, 2060 Antwerp, Belgium.

We publish all received postcards [on our company website](https://jarhen.be/en/opensource/postcards).
...
Jarhen is a web design agency based in Antwerp, Belgium. You'll find an overview of all our open source projects [on our website](https://jarhen.be/opensource).

Does your business depend on our contributions? Reach out and support us on [Patreon](https://www.patreon.com/jarhen). 
